...
#   - This script assumes that ../dist/docker/repaired_wheels has the wheel(s) built for all versions to be tested
...
for PYTHON_VERSION in ${PYTHON_VERSIONS}; do
    ...
    CONNECTOR_WHL=$(ls $CONNECTOR_DIR/dist/docker/repaired_wheels/snowflake_connector_python*cp${SHORT_VERSION}*manylinux2010*.whl | sort -r | head -n 1)
