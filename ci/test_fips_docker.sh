...
if [[ ! -d "$CONNECTOR_DIR/dist/" ]] || [[ $(ls $CONNECTOR_DIR/dist/*cp310*manylinux2014*.whl) == '' ]]; then
  echo "Missing wheel files, going to compile Python connector in Docker..."
  $THIS_DIR/build_docker.sh 3.10
  cp $CONNECTOR_DIR/dist/repaired_wheels/*cp310*manylinux2014*.whl $CONNECTOR_DIR/dist/
fi
