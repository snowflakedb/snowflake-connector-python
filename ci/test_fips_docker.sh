...
if [[ ! -d "$CONNECTOR_DIR/dist/" ]] || [[ $(ls $CONNECTOR_DIR/dist/*cp38*manylinux2010*.whl) == '' ]]; then
  echo "Missing wheel files, going to compile Python connector in Docker..."
  $THIS_DIR/build_docker.sh 3.8
  cp $CONNECTOR_DIR/dist/repaired_wheels/*cp38*manylinux2010*.whl $CONNECTOR_DIR/dist/
fi
