...
/**
 * A RAII class to help us acquire the python GIL. The semantics are like std::unique_lock.
 * We have to acquire the python GIL every time we call a Python/C API to ensure there is only one python thread running all the time.
 */
class PyUniqueLock
{
public:
