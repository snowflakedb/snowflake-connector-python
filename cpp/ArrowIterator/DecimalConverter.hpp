...
{
public:
    DecimalFromIntConverter(std::shared_ptr<arrow::Array> array, int precision, int scale)
...
private:
    ...
    int m_precision;    // looks like the precision here is not useful, and this will be removed soon when it's been confirmed
...
PyObject* DecimalFromIntConverter<T>::toPyObject(int64_t rowIndex)
{
    if (m_array->IsValid(rowIndex))
    {
        ...
        py::UniqueRef decimal(PyObject_CallFunction(m_decimalConstructor().get(), "L", val));
