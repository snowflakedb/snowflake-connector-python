...
class CRLValidator:
    ...
    def validate_connection(self, connection: SSLConnection | SSLObject) -> bool:
        """
        Validate an connection against CRLs.

        This method extracts certificate chains from the connection and validates them
        against Certificate Revocation Lists (CRLs).

        Args:
            connection: OpenSSL or SSLObject connection object
        ...
        try:
            if isinstance(connection, SSLObject):
                # AIOHTTP SSLObject case
                cert_chain = connection._sslobj.get_unverified_chain()
                cert_der_chain = [
                    ssl.PEM_cert_to_DER_cert(c.public_bytes()) for c in cert_chain
                ]
            else:
                # OpenSSL SSLConnection case
                cert_chain = connection.get_peer_cert_chain()
                cert_der_chain = [
                    dump_certificate(FILETYPE_ASN1, c) for c in cert_chain
                ]
    ...
    def _extract_certificate_chains_from_connection(
        self, cert_chain: list[bytes]
    ) -> list[list[x509.Certificate]]:
        """Extract certificate chains from list of certs.

        Args:
            cert_chain: List of DER-encoded certificates from the connection
        ...
        logger.debug("Extracted %d certificates for CRL validation", len(x509_chain))
