...
class ConnectionDiagnostic:
    ...
    def __init__(self,
        ...
        self.proxy_host, self.proxy_port, self.proxy_user, self.proxy_password = self.__parse_proxy(proxy_url)
        ...
        if self.full_connection_diag_log_path is None:
            ...
            if not self.full_connection_diag_log_path.is_absolute():
                logger.warning(f"Path {self.full_connection_diag_log_path} for connection test is not absolute.")
                self.full_connection_diag_log_path = Path(self.tmpdir)
        ...
        try:
            ...
            if "BEGIN CERTIFICATE" in certificate:
                ...
                if 'subjectAltName' in extension_data:
                    self.__append_message(host_type, f"{host}: subjectAltName: {extension_data['subjectAltName']}")
    ...
    def __check_for_proxies(self):
        ...
        self.__append_message(host_type, f"Proxies with Env vars removed(SYSTEM PROXIES): {system_proxies}")
        ...
        if 'https' in system_proxies.keys():
            ...
            self.__append_message(host_type, f"""If there are failures, try using the SYSTEM PROXY: On Windows, do
        ...
        if not re.search(check_pattern, issuer):
            self.__append_message(host_type, f"There is likely a proxy because the issuer for {test_host} is "
                                             f"not correct. Got {issuer} and expected one of {cert_authorities}")
        ...
        try:
            ...
            if self.proxy_host is None:
                ...
                default_headers = urllib3.util.make_headers(proxy_basic_auth=f"{self.proxy_user}:{self.proxy_password}")
                http = urllib3.ProxyManager(os.environ['HTTPS_PROXY'], proxy_headers=default_headers, timeout=10.0,
                                            cert_reqs=cert_reqs)
            resp = http.request('GET', "https://ireallyshouldnotexistatallanywhere.com", timeout=10.0)

            # squid does not throw exception.  Check HTML
            if "does not exist" in str(resp.data.decode('utf-8')):
                self.__append_message(host_type, f"It is likely there is a proxy based on HTTP response.")
        except Exception as e:
            if "NewConnectionError" in str(e):
                self.__append_message(host_type, f"Proxy check using invalid URL did not show proxy: Review result, "
                    ...
                    if registry_key:
                        ...
                        if "internet settings" in str(registry_key).lower() or "wpad" in str(registry_key).lower():
                            ...
                            if 'AutoConfigURL' in values.keys():
                                ...
                                self.__append_message(host_type, f"There may be a proxy: Found a Wpad in Windows "
                                ...
                                try:
                                    ...
                                    self.__append_message(host_type, f"Wpad request returned possible proxy: {proxy_info}")
                                except:
                                    pass

                            elif 'ProxyServer' in values.keys():
                                proxy = values['ProxyServer'][0]
                                self.__append_message(host_type, f"There may be a proxy: Found a proxy server in "
                                ...
                                if proxy_enable == 1:
                                    self.__append_message(host_type, f"There may be a proxy: Proxy is enabled per the "
                                                                     f"registry: hkey: {hkey} : {new_registry_key_str}"
                                                                     f" : ProxyEnable: {proxy_enable}")
                            else:
                                self.__append_message(host_type, f"Found Wpad key in registry: Most likely nothing, "
