    ...
    **kwargs: Any,
    ...
    try:
        if overwrite and (not auto_create_table):
            truncate_sql = (
                "TRUNCATE TABLE identifier(?) /* Python:snowflake.connector.aio._pandas_tools.write_pandas() */"
            )
            params = (target_table_location,)
        ...
        await cursor._log_telemetry_job_data(TelemetryField.PANDAS_WRITE, TelemetryData.TRUE)
        await cursor.close()
