...
from .. import DatabaseError, Error, OperationalError, ProgrammingError, proxy
...
from ..network import REQUEST_ID, ReauthenticationRequest
from ..sqlstate import SQLSTATE_CONNECTION_NOT_EXISTS
from ..time_util import get_time_millis
from .auth import AuthAsync, AuthByDefaultAsync, AuthByPluginAsync
from .cursor_async import SnowflakeCursorAsync
from .network_async import SnowflakeRestfulAsync

logger = getLogger(__name__)


class SnowflakeConnectionAsync(SnowflakeConnection):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # perform connect
        # TODO: should we create an event loop here?
        self._conn_parameters = self._init_connection(
    ...
    @property
    def rest(self) -> SnowflakeRestfulAsync | None:
        return self._rest
    ...
    async def connect(self) -> None:
        ...
        if len(self._conn_parameters) > 0:
            self._config(**self._conn_parameters)
    ...
    async def __open_connection(self):
        ...
        if self._session_token and self._master_token:
            ...
            if self.auth_class is not None:
                ...
                self.auth_class = AuthByDefaultAsync(
    ...
    def cursor(
        self, cursor_class: type[SnowflakeCursorAsync] = SnowflakeCursorAsync
    ) -> SnowflakeCursorAsync:
        logger.debug("cursor")
        if not self.rest:
            Error.errorhandler_wrapper(
                self,
    ...
    @property
    def auth_class(self) -> AuthByPluginAsync | None:
        return self._auth_class

    @auth_class.setter
    def auth_class(self, value: AuthByPluginAsync) -> None:
        if isinstance(value, AuthByPluginAsync):
    ...
    async def _authenticate(self, auth_instance: AuthByPluginAsync):
