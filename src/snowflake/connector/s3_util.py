...
class SnowflakeS3RestClient:
    def __init__(self, stage_info, use_accelerate_endpoint=False):
        ...
        if stage_info["endPoint"]:
            ...
            self.endpoint = (
                f"https://{{bucket_name}}.s3.{self.region_name}.amazonaws.com"
            )
    ...
    def construct_canonicalized_element(
        bucket_name: str = None, request_uri: str = "", subresource: str = None
    ):
        res = ""
        if bucket_name:
            res += f"/{bucket_name}"
    ...
    def get_file_header(self, meta: "SnowflakeFileMeta", filename: str):
        ...
        string_to_sign = self.construct_string_to_sign(
            "HEAD", _resource, _headers, amzdate
        )
        signature = self.sign(
            self.aws_secret_access_key.encode("UTF-8"), string_to_sign
        )

        authorization_header = (
            "AWS" + " " + self.aws_access_key_id + ":" + signature.decode()
        )
        ...
        if r.status_code == 200:
            ...
            if err_code == EXPIRED_TOKEN:
                ...
                logger.debug(
                    f"Bad request, token needs to be renewed: {err_message}. "
                    f"bucket: {s3location.bucket_name}, path: {s3path}"
                )
                meta.result_status = ResultStatus.RENEW_TOKEN
                return None
            logger.debug(
                f"Failed to get metadata for {s3location.bucket_name}, {s3path}: {err_message}"
            )
    ...
    def upload_file(
        ...
        string_to_sign = SnowflakeS3RestClient.construct_string_to_sign(
            "PUT",
            _resource,
            _headers,
            amzdate,
        ...
        signature = SnowflakeS3RestClient.sign(
            self.aws_secret_access_key.encode("UTF-8"), string_to_sign
        )
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if err_code == EXPIRED_TOKEN:
                    ...
                    return
                logger.debug(
                    f"Failed to upload a file: {data_file}, err: {err_code} {err_message}"
                )
    ...
    def _native_download_file(self, meta, full_dst_file_name, max_concurrency):
        s3location = self.extract_bucket_name_and_path(
            meta.client_meta.stage_info["location"]
        )
        ...
        string_to_sign = self.construct_string_to_sign(
            "GET", _resource, _headers, amzdate
        )
        signature = self.sign(
            self.aws_secret_access_key.encode("UTF-8"), string_to_sign
        )
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if err_code == EXPIRED_TOKEN:
                    ...
                    logger.debug(
                        f"Failed to download a file: {full_dst_file_name}, err: {err_code} {err_message}"
                    )
                    raise S3RestCallFailedError
        except OpenSSL.SSL.SysCallError as err:
            meta.last_error = err
    ...
    def transfer_accelerate_config(self) -> bool:
        ...
        _resource = self.construct_canonicalized_element(
            bucket_name=s3location.bucket_name, request_uri="?accelerate"
        )
        string_to_sign = SnowflakeS3RestClient.construct_string_to_sign(
            "GET", _resource, _headers, amzdate
        )
        ...
        if r.status_code == 200:
            ...
            (
                err_code,
                err_message,
            ) = SnowflakeS3RestClient.extract_error_from_xml_response(r.text)
            if err_code == "AccessDenied":
                logger.debug(
                    f"Cannot GET bucket accelerate configuration: {err_message}"
                )
            else:
                logger.debug(
                    f"Unknown error when GET bucket accelerate configuration, {err_code}, {err_message}"
                )
            return False
