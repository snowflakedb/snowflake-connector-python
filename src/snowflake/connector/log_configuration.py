...
class EasyLoggingConfigPython:
    ...
    def parse_config_file(self, skip_config_file_permissions_check: bool = False):
        CONFIG_MANAGER.read_config(skip_file_permissions_check=skip_config_file_permissions_check)
        ...
        if log := data.get("log"):
            ...
            if not os.path.isabs(self.path):
                raise FileNotFoundError(f"Log path must be an absolute file path: {self.path}")
            # if log path does not exist, create it, else check accessibility
            if not os.path.exists(self.path):
                os.makedirs(self.path, exist_ok=True)
            elif not os.access(self.path, os.R_OK | os.W_OK):
                raise PermissionError(f"log path: {self.path} is not accessible, please verify your config file")
    ...
    def create_log(self):
        if self.save_logs:
            ...
            for logger_name in ["snowflake.connector", "botocore", "boto3"]:
                ...
                ch = TimedRotatingFileHandler(os.path.join(self.path, LOG_FILE_NAME), when="midnight")
