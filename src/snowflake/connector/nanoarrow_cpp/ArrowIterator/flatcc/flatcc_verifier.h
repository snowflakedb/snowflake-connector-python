...
#define FLATCC_VERIFY_ERROR_MAP(XX)                                           \
  ...
  XX(table_header_out_of_range_or_unaligned,                                  \
     "table header out of range or unaligned")                                \
  XX(vector_header_out_of_range_or_unaligned,                                 \
     "vector header out of range or unaligned")                               \
  XX(string_header_out_of_range_or_unaligned,                                 \
     "string header out of range or unaligned")                               \
  XX(offset_out_of_range, "offset out of range")                              \
  XX(table_offset_out_of_range_or_unaligned,                                  \
     "table offset out of range or unaligned")                                \
  XX(table_size_out_of_range, "table size out of range")                      \
  XX(type_field_absent_from_required_union_field,                             \
     "type field absent from required union field")                           \
  XX(type_field_absent_from_required_union_vector_field,                      \
     "type field absent from required union vector field")                    \
  XX(union_cannot_have_a_table_without_a_type,                                \
     "union cannot have a table without a type")                              \
  XX(union_type_NONE_cannot_have_a_value,                                     \
     "union value field present with type NONE")                              \
  ...
  XX(vtable_offset_out_of_range_or_unaligned,                                 \
     "vtable offset out of range or unaligned")                               \
  XX(vtable_size_out_of_range_or_unaligned,                                   \
     "vtable size out of range or unaligned")                                 \
  XX(vtable_size_overflow, "vtable size overflow")                            \
  XX(union_element_absent_without_type_NONE,                                  \
     "union element absent without type NONE")                                \
  XX(union_element_present_with_type_NONE,                                    \
     "union element present with type NONE")                                  \
  XX(union_vector_length_mismatch,                                            \
     "union type and table vectors have different lengths")                   \
  XX(union_vector_verification_not_supported,                                 \
     "union vector verification not supported")                               \
 ...
 */
typedef struct flatcc_table_verifier_descriptor
...
typedef struct flatcc_union_verifier_descriptor
 ...
 *
 * If `fid` is null, the identifier is not checked and is allowed to be entirely
 * absent.
 *
...
int flatcc_verify_vector_field(flatcc_table_verifier_descriptor_t *td,
                               ...
                               size_t elem_size, uint16_t align,
                               size_t max_count);
