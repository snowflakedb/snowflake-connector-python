...
from .util_text import construct_hostname, parse_account
...
class SnowflakeConnection(SnowflakeConnectionBase):
    ...
    def _update_parameters(
        ...
        if PARAMETER_CLIENT_TELEMETRY_ENABLED in parameters:
            self.telemetry_enabled = parameters[PARAMETER_CLIENT_TELEMETRY_ENABLED]

        if PARAMETER_CLIENT_TELEMETRY_OOB_ENABLED in parameters:
            if parameters[PARAMETER_CLIENT_TELEMETRY_OOB_ENABLED]:
                TelemetryService.get_instance().enable()
            else:
                TelemetryService.get_instance().disable()

        if (
            PARAMETER_CLIENT_SESSION_KEEP_ALIVE in parameters
            and self.client_session_keep_alive is None
        ):
            # Only set if the local config is None.
            # Always give preference to user config.
            self.client_session_keep_alive = parameters[
                PARAMETER_CLIENT_SESSION_KEEP_ALIVE
            ]

        if (
            PARAMETER_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY in parameters
            and self.client_session_keep_alive_heartbeat_frequency is None
        ):
            self.client_session_keep_alive_heartbeat_frequency = parameters[
                PARAMETER_CLIENT_SESSION_KEEP_ALIVE_HEARTBEAT_FREQUENCY
            ]

        if PARAMETER_CLIENT_PREFETCH_THREADS in parameters:
            self.client_prefetch_threads = parameters[PARAMETER_CLIENT_PREFETCH_THREADS]

        if PARAMETER_ENABLE_STAGE_S3_PRIVATELINK_FOR_US_EAST_1 in parameters:
            self.enable_stage_s3_privatelink_for_us_east_1 = parameters[
                PARAMETER_ENABLE_STAGE_S3_PRIVATELINK_FOR_US_EAST_1
            ]
