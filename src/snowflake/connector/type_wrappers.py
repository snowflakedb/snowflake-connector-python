...
class snowflake_map(snowflake_type_wrapper, dict):
    def __init__(self, seq=None, **kwargs):
        ...
        if seq:
            ...
            if keys:
                self.key_type = converter.snowflake_type(keys[0])
                if not all(converter.snowflake_type(k) == self.key_type for k in keys):
                    raise ValueError("All keys must have the same snowflake_type.")
            if values:
                self.value_type = converter.snowflake_type(values[0])
                if not all(
                    converter.snowflake_type(v) == self.value_type for v in values
                ):
                    raise ValueError("All values must have the same snowflake_type.")

    def __setitem__(self, key, value):
        key_type = converter.snowflake_type(key)
        value_type = converter.snowflake_type(value)
