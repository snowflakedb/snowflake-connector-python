...
class SnowflakeS3RestClient(SnowflakeStorageClient):
    ...
    def transfer_accelerate_config(self, use_accelerate_endpoint: bool | None = None) -> bool:
        ...
        if self.use_s3_regional_url:
            ...
            if use_accelerate_endpoint is None:
                if str(self.s3location.bucket_name).lower().startswith("sfc-"):
                    ...
                    use_accelerate_endpoint = self._get_bucket_accelerate_config(self.s3location.bucket_name)
    ...
    @staticmethod
    def _construct_canonicalized_and_signed_headers(headers: dict[str, str | list[str]]) -> tuple[str, str]:
    ...
    def _construct_canonical_request_and_signed_headers(
        ...
        """
        canonical_query_string = "&".join("=".join([k, v]) for k, v in sorted(query_parts.items(), key=itemgetter(0)))
        (
            canonical_headers,
            signed_headers,
        ) = SnowflakeS3RestClient._construct_canonicalized_and_signed_headers(canonical_headers)
    ...
    def _send_request_with_authentication_and_retry(
        ...
        return self._send_request_with_retry(verb, generate_authenticated_url_and_args_v4, retry_id)
    ...
    def get_file_header(self, filename: str) -> FileHeader | None:
        ...
        response = self._send_request_with_authentication_and_retry(url=url, verb="HEAD", retry_id=retry_id)
