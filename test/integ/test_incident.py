...
@pytest.mark.internal
@patch("snowflake.connector.incident.CLS_BLACKLIST", frozenset())
@pytest.mark.parametrize("app_name", ["asd", "mark"])
@pytest.mark.skipif(
    not RUNNING_AGAINST_LOCAL_SNOWFLAKE, reason="local Snowflake is necessary"
)
def test_reporting_values(conn_cnx, app_name):
    with patch.dict(converter.PYTHON_TO_SNOWFLAKE_TYPE, {"nonetype": None}):
        try:
            with conn_cnx(
                paramstyle="qmark", internal_application_name=app_name
