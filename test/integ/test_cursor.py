...
def test_insert_select(conn_cnx, created_table):
    ...
    with conn_cnx() as cnx, cnx.cursor() as c:
        c.execute(f"insert into {table_name}(aa) values(123456)," "(98765),(65432)")
        cnt = 0
        for rec in c:
            cnt += int(rec[0])
...
def test_insert_and_select_by_separate_connection(conn_cnx, created_table):
    """Inserts a record and select it by a separate connection."""
    table_name = created_table
    with conn_cnx() as cnx:
        result = cnx.cursor().execute(f"insert into {table_name}(aa) values({1234})")
        cnt = 0
        for rec in result:
            cnt += int(rec[0])
...
def test_insert_timestamp_select(conn_cnx, created_table):
    ...
    with conn_cnx() as cnx:
        ...
        with cnx.cursor() as c:
            fmt = (
                f"insert into {table_name}(aa, tsltz, tstz, tsntz, dt, tm) "
...
def test_struct_time(conn_cnx, created_table):
    ...
    with conn_cnx() as cnx, cnx.cursor() as c:
        ...
        result = cnx.cursor().execute(f"select aa, tsltz from {table_name}")
...
@pytest.mark.parametrize(
    "value", [b"\x00\xFF\xA1\xB2\xC3", bytearray(b"\x00\xFF\xA1\xB2\xC3")]
)
...
"""
    ...
    with conn_cnx() as cnx, cnx.cursor() as c:
        current_time = datetime.now()
        fmt = (
            f"insert into {table_name}(created_at, data) "
        ...
        c.execute(
            fmt,
            {
                ...
                "data": (
                    '{"SESSION-PARAMETERS":{' '"TIMEZONE":"UTC", "SPECIAL_FLAG":true}}'
                ),
            },
        )
        cnt = 0
        ...
        result = cnx.cursor().execute(f"select created_at, data from {table_name}")
...
def test_closed_cursor(conn_cnx):
    ...
    with conn_cnx() as cnx:
        ...
        cur.close()
        with pytest.raises(
            snowflake.connector.Error, match=r"Cursor is closed in execute"
        ) as err:
...
def test_binding_negative(negative_conn_cnx):
    ...
    with negative_conn_cnx() as cnx:
        with pytest.raises(TypeError):
            cnx.cursor().execute(f"INSERT INTO {table_name}(aa) VALUES(%s)", (1, 2, 3))
        with pytest.raises(errors.ProgrammingError):
            cnx.cursor().execute(f"INSERT INTO {table_name}(aa) VALUES(%s)", ())
        with pytest.raises(errors.ProgrammingError):
            cnx.cursor().execute(f"INSERT INTO {table_name}(aa) VALUES(%s)", (["a"],))
...
def test_check_can_use_arrow_resultset(conn_cnx, caplog):
    ...
    with conn_cnx() as cnx, cnx.cursor() as cur:
        with mock.patch(
            "snowflake.connector.cursor.CAN_USE_ARROW_RESULT", True
...
def test_query_cancellation(conn_cnx):
    ...
    with conn_cnx() as cnx, cnx.cursor() as cur:
        cur.execute(
            "select max(seq8()) from table(generator(timeLimit=>30));", _no_results=True
        )
        sf_qid = cur.sfqid
...
def test__log_telemetry_job_data(conn_cnx, caplog):
    ...
    with conn_cnx() as con, con.cursor() as cur:
        with mock.patch.object(cur, "_connection", None), caplog.at_level(
            logging.DEBUG, "snowflake.connector"
        ):
