...
async def test_client_session_keep_alive(db_parameters, conn_cnx):
...
async def set_backend_client_session_keep_alive(
    ...
    await admin_cnx.cursor().execute(query)
...
async def test_htap_optimizations(db_parameters: object, conn_cnx) -> None:
    ...
    async with conn_cnx("admin") as admin_cnx:
        try:
            await admin_cnx.cursor().execute(
                f"CREATE WAREHOUSE IF NOT EXISTS {test_wh}"
            )
            await admin_cnx.cursor().execute(f"USE WAREHOUSE {test_wh}")
            await admin_cnx.cursor().execute(f"CREATE DATABASE IF NOT EXISTS {test_db}")
            await admin_cnx.cursor().execute(
            ...
            # select * from table
            ret = await (
                await admin_cnx.cursor().execute("select * from testtable1")
            ).fetchall()
            ...
            await admin_cnx.cursor().execute(query)
            await admin_cnx.cursor().execute(f"DROP SCHEMA IF EXISTS {test_schema}")
            await admin_cnx.cursor().execute(f"DROP DATABASE IF EXISTS {test_db}")
            await admin_cnx.cursor().execute(f"DROP WAREHOUSE IF EXISTS {test_wh}")
