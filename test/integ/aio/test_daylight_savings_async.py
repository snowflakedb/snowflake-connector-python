...
async def _insert_timestamp(ctx, table, tz, dt):
    ...
    result = await (await ctx.cursor().execute(f"SELECT * FROM {table}")).fetchone()
    ...
    print("#####")
    assert result[0] == ts
    await ctx.cursor().execute(f"DELETE FROM {table}")


async def test_daylight_savings_in_TIMESTAMP_LTZ(conn_cnx, db_parameters):
    async with conn_cnx() as ctx:
        await ctx.cursor().execute(
        ...
        try:
            ...
            await _insert_timestamp(ctx, db_parameters["name"], "Australia/Sydney", dt)
            dt = datetime(year=2016, month=3, day=13, hour=8, minute=39, second=23)
            await _insert_timestamp(ctx, db_parameters["name"], "Europe/Paris", dt)
            dt = datetime(year=2016, month=3, day=13, hour=8, minute=39, second=23)
            await _insert_timestamp(ctx, db_parameters["name"], "UTC", dt)

            dt = datetime(year=2016, month=3, day=13, hour=1, minute=14, second=8)
            await _insert_timestamp(ctx, db_parameters["name"], "America/New_York", dt)

            dt = datetime(year=2016, month=3, day=12, hour=22, minute=32, second=4)
            await _insert_timestamp(ctx, db_parameters["name"], "US/Pacific", dt)
