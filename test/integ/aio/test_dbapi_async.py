...
async def drop_dbapi_tables(conn_cnx):
    async with conn_cnx() as cnx:
        async with cnx.cursor() as cursor:
            for ddl in (TABLE1, TABLE2):
                dropsql = f"drop table if exists {ddl}"
                await cursor.execute(dropsql)


async def executeDDL1(cursor):
    await cursor.execute(f"create or replace table {TABLE1} (name string)")


async def executeDDL2(cursor):
    await cursor.execute(f"create or replace table {TABLE2} (name string)")


@pytest.fixture()
async def conn_local(request, conn_cnx):
    async def fin():
...
async def _paraminsert(cur):
    ...
    await cur.execute(f"insert into {TABLE1} values ('string inserted into table')")
    assert cur.rowcount in (-1, 1)

    await cur.execute(
        f"insert into {TABLE1} values (%(dbapi_ddl2)s)", {TABLE2: "Cooper's"}
    )
    assert cur.rowcount in (-1, 1)

    await cur.execute(f"select name from {TABLE1}")
...
async def test_connect(conn_cnx):
    async with conn_cnx():
        pass


async def test_apilevel():
    try:
...
""".format(
        ...
        await cnx.rollback()
        dbapi_rollback = await (
            await cur.execute("select count(*) from {}".format(db_parameters["name"]))
        ).fetchone()
        assert dbapi_rollback[0] == 0, "transaction not rolled back"
        await cur.execute("drop table {}".format(db_parameters["name"]))
        await cur.close()


async def test_cursor(conn_cnx):
    async with conn_cnx() as cnx:
        try:
            cur = cnx.cursor()
        finally:
...
async def test_cursor_isolation(conn_local):
    async with conn_local() as con:
        # two cursors from same connection have transaction isolation
        cur1 = con.cursor()
        cur2 = con.cursor()
        await executeDDL1(cur1)
        await cur1.execute(
            f"insert into {TABLE1} values ('string inserted into table')"
        )
        await cur2.execute(f"select name from {TABLE1}")
        dbapi_ddl1 = await cur2.fetchall()
        assert len(dbapi_ddl1) == 1
        assert len(dbapi_ddl1[0]) == 1
        assert dbapi_ddl1[0][0], "string inserted into table"


async def test_description(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
...
async def test_rowcount(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
...
async def test_close(db_parameters):
    ...
    with pytest.raises(errors.Error) as e:
        await cur.execute(f"create or replace table {TABLE1} (name string)")
    assert (
...
async def test_execute(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        await _paraminsert(cur)


async def test_executemany(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        await executeDDL1(cur)
        ...
        await cur.executemany(
            "insert into %s values (%%(dbapi_ddl2)s)" % (TABLE1), margs
        )
...
async def test_fetchone(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        # SNOW-13548 - disabled
        ...
        await cur.execute("insert into %s values ('Row 1'),('Row 2')" % TABLE1)
        await cur.execute("select name from %s order by 1" % TABLE1)
...
async def test_fetchmany(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        ...
        await executeDDL2(cur)
        await cur.execute("select name from %s" % TABLE2)
...
async def test_fetchall(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        # disable due to SNOW-13648
...
async def test_mixedfetch(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        await executeDDL1(cur)
        for sql in _populate():
...
async def test_arraysize(conn_cnx):
    async with conn_cnx() as con:
        cur = con.cursor()
        assert hasattr(cur, "arraysize"), "cursor.arraysize must be defined"


async def test_setinputsizes(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        cur.setinputsizes((25,))
        await _paraminsert(cur)  # Make sure cursor still works


async def test_setoutputsize_basic(conn_local):
    # Basic test is to make sure setoutputsize doesn't blow up
    async with conn_local() as con:
        cur = con.cursor()
        cur.setoutputsize(1000)
        cur.setoutputsize(2000, 0)
        await _paraminsert(cur)  # Make sure the cursor still works


async def test_description2(conn_local):
    try:
        async with conn_local() as con:
...
"""
                ...
                await cursor.execute("select * from test_description")
                await cursor.fetchone()
                assert cursor.description == expected, "cursor.description is incorrect"
    finally:
        async with conn_local() as con:
            async with con.cursor() as cursor:
                await cursor.execute("drop table if exists test_description")
                await cursor.execute(
                    "alter session set timestamp_input_format = default"
                )


async def test_closecursor(conn_cnx):
    async with conn_cnx() as cnx:
        cursor = cnx.cursor()
        await cursor.close()
...
async def test_None(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        await executeDDL1(cur)
        await cur.execute("insert into %s values (NULL)" % TABLE1)
        await cur.execute("select name from %s" % TABLE1)
...
async def test_substring(conn_local):
    async with conn_local() as con:
        cur = con.cursor()
        await executeDDL1(cur)
        args = {"dbapi_ddl2": '"" "\'",\\"\\""\'"'}
...
async def test_escape(conn_local):
    teststrings = [
        "abc\ndef",
        "abc\\ndef",
        "abc\\\ndef",
...
async def test_callproc(conn_local):
    ...
    async with conn_local() as con:
        ...
        ret = await cur.callproc(name_sp, (message,))
        assert ret == (message,) and await cur.fetchall() == [(message,)]


@pytest.mark.skipolddriver
@pytest.mark.parametrize("paramstyle", ["pyformat", "qmark"])
async def test_callproc_overload(conn_cnx, paramstyle):
    """Test calling stored procedures overloaded with different input parameters and returns."""
    name_sp = random_string(5, "test_stored_procedure_")
    async with conn_cnx(paramstyle=paramstyle) as cnx:
        async with cnx.cursor() as cursor:
            await cursor.execute(
                f"""
            ...
            ret = await cursor.callproc(name_sp, ("str", 1, "2022-02-22"))
            assert ret == ("str", 1, "2022-02-22") and await cursor.fetchall() == [
                ("teststring",)
            ]

            ret = await cursor.callproc(name_sp, (0.99, "c"))
            assert ret == (0.99, "c") and await cursor.fetchall() == [(1.23,)]

            ret = await cursor.callproc(name_sp, (True,))
            assert ret == (True,) and await cursor.fetchall() == [(1, "a"), (2, "b")]

            ret = await cursor.callproc(name_sp)
            assert ret == () and await cursor.fetchall() == [(True,)]


@pytest.mark.skipolddriver
async def test_callproc_invalid(conn_cnx):
    ...
    async with conn_cnx() as cnx:
        async with cnx.cursor() as cur:
            ...
            with pytest.raises(TypeError):
                await cur.callproc(name_sp, message)
