...
"""
        ...
        cnx.rollback()
        dbapi_rollback = cur.execute(f"select count(*) from {table_name}").fetchone()
...
def test_cursor_isolation(conn_cnx, request):
    ...
    with conn_cnx() as con:
        ...
        cur1.execute(f"insert into {table_1} values ('string inserted into table')")
...
def test_fetchone(conn_cnx, request):
    ...
    with conn_cnx() as con:
        ...
        request.addfinalizer(drop_table(conn_cnx, table_name))
        cur.execute(f"select name from {table_name}")
        cur.execute(f"insert into {table_name} values ('Row 1'),('Row 2')")
        cur.execute(f"select name from {table_name} order by 1")
        r = cur.fetchone()
...
def _populate(cursor, table_name):
    ...
    cursor.executemany(
        f"insert into {table_name} values(%(value)s)", [{"value": s} for s in SAMPLES]
    )
...
def test_setinputsizes(conn_cnx):
...
def test_setoutputsize(conn_cnx):
...
def test_description_2(conn_cnx, request):
    ...
    with conn_cnx() as con, con.cursor() as cur:
        ...
        cur.execute(f"select name from {table_name}")
        assert len(cur.description) == 1, "cursor.description returns too many columns"
        assert (
            len(cur.description[0]) == 7
        ), "cursor.description[x] tuples must have 7 elements"
        assert (
...
"""
            ...
            request.addfinalizer(
                execute(conn_cnx, "alter session set timestamp_input_format = default")
            )
...
"""
            ...
            cursor.execute(f"select * from {description_table}")
            cursor.fetchone()
...
def test_substring(conn_cnx, request):
    ...
    with conn_cnx() as con, con.cursor() as cur:
        ...
        args = {"dbapi_ddl2": '"" "\'",\\"\\""\'"'}
        cur.execute(f"insert into {table_name} values (%(dbapi_ddl2)s)", args)
        cur.execute(f"select name from {table_name}")
        res = cur.fetchall()
        dbapi_ddl2 = res[0][0]
        assert (
...
def test_escape(conn_cnx, request):
    values = [
        "abc\ndef",
        "abc\\ndef",
        "abc\\\ndef",
    ...
    with conn_cnx() as con, con.cursor() as cur:
        ...
        for i in range(len(values)):
            cur.execute(
                f"insert into {table_name} values ({i} , %(value)s)",
                {"value": values[i]},
            )
