...
def ingest_data(request, conn_cnx, db_parameters):
    ...
    """
        ...
        first_val = (
            cnx.cursor()
            .execute(f"select c0 from {table_name} order by 1 limit 1")
            .fetchone()[0]
        )
        last_val = (
            cnx.cursor()
            .execute(f"select c9 from {table_name} order by 1 desc limit 1")
            .fetchone()[0]
        )
...
def test_query_large_result_set_n_threads(conn_cnx, ingest_data, num_threads):
