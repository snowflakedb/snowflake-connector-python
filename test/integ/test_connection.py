...
def test_drop_create_user(conn_cnx, db_parameters):
    ...
    with conn_cnx() as cnx, cnx.cursor() as csr:
        ...
        csr.execute(
            f"grant all on database {db_parameters['database']} to role snowdog_role"
        )
        csr.execute(
            f"grant all on schema {db_parameters['schema']} to role snowdog_role"
        )
...
def test_privatelink(db_parameters):
    ...
    with mock.patch.dict(
        "os.environ", {"SF_OCSP_FAIL_OPEN": "false", "SF_OCSP_DO_RETRY": "false"}
    ):
        with pytest.raises(OperationalError):
            snowflake.connector.connect(
                account="testaccount",
