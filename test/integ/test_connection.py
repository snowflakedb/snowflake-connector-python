...
@pytest.mark.skipolddriver
def test_ocsp_mode_in_secure(db_parameters, is_public_test):
    import snowflake.connector.ocsp_snowflake

    # reset cache to None first so that to clean cache generated by previous test running
    snowflake.connector.ocsp_snowflake.OCSP_RESPONSE_VALIDATION_CACHE = None
    with snowflake.connector.connect(
    ...
    if not is_public_test:
        # non-public test account uses http which doesn't use any certificate
        assert snowflake.connector.ocsp_snowflake.OCSP_RESPONSE_VALIDATION_CACHE is None
    else:
        assert (
