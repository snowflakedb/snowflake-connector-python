...
def test_daylight_savings_in_TIMESTAMP_LTZ(conn_cnx, request):
    ...
    with conn_cnx() as cnx, cnx.cursor() as cur:
        cur.execute(
            f"CREATE TABLE {table_name} (c1 timestamp_ltz, "
            f"c2 timestamp_ltz, "
            f"c3 timestamp_ltz, "
            f"c4 timestamp_ltz, "
            f"c5 timestamp_ltz)"
    ...
    data = [
        (
            datetime(year=2016, month=3, day=13, hour=18, minute=47, second=32),
            "Australia/Sydney",
        ),
        (
            datetime(year=2016, month=3, day=13, hour=8, minute=39, second=23),
            "Europe/Paris",
        ),
        (datetime(year=2016, month=3, day=13, hour=8, minute=39, second=23), "UTC"),
        (
            datetime(year=2016, month=3, day=13, hour=1, minute=14, second=8),
            "America/New_York",
        ),
        (
            datetime(year=2016, month=3, day=12, hour=22, minute=32, second=4),
            "US/Pacific",
        ),
    ]
    ...
    insert = (
        ...
        + ", ".join("%s" for _ in range(len(ts)))
        + ")"
    )
    with conn_cnx() as cnx, cnx.cursor() as cur:
        cur.execute(insert, ts)
