...
async def test_put_get_small_data_via_user_stage(
...
async def _put_get_user_stage_s3_regional_url(
    tmpdir,
    conn_cnx,
    db_parameters,
    number_of_files=1,
    ...
    try:
        put_cursor = await _put_get_user_stage(
            tmpdir,
            conn_cnx,
            number_of_files,
            number_of_lines,
...
"""
    ...
    try:
        async with conn_cnx() as cnx:
            ...
            await cnx.cursor().execute(f"rm @{stage_name}")

            put_cursor = cnx.cursor()
            await put_async(
                put_cursor, files, stage_name, from_path, file_stream=file_stream
            )
            await cnx.cursor().execute(f"copy into {random_str} from @{stage_name}")
            c = cnx.cursor()
            try:
                await c.execute(f"select count(*) from {random_str}")
                rows = 0
                async for rec in c:
                    rows += rec[0]
                ...
                await c.close()
            await cnx.cursor().execute(f"rm @{stage_name}")
            await cnx.cursor().execute(f"copy into @{stage_name} from {random_str}")
            tmp_dir_user = str(tmpdir.mkdir("put_get_stage"))
            await cnx.cursor().execute(f"get @{stage_name}/ file://{tmp_dir_user}/")
        ...
        async with conn_cnx() as cnx:
            await cnx.cursor().execute(f"rm @{stage_name}")
            await cnx.cursor().execute(f"drop stage if exists {stage_name}")
            await cnx.cursor().execute(f"drop table if exists {random_str}")
    return put_cursor
...
"""
    ...
    try:
        async with conn_cnx() as cnx:
            c = cnx.cursor()
            try:
                async for rec in await c.execute(f"rm @{stage_name}"):
            ...
            deleted_cnt += 1
            await cnx.cursor().execute(f"rm @{stage_name}/file1")
            deleted_cnt += 1
            await cnx.cursor().execute(f"rm @{stage_name}/file2")
            deleted_cnt += 1

            success_cnt = 0
            skipped_cnt = 0
            async with cnx.cursor() as c:
                async for rec in await c.execute(
...
@pytest.mark.aws
async def test_get_data_user_stage(
    is_public_test,
    tmpdir,
    conn_cnx,
):
    ...
    for elem in test_data:
        await _put_list_rm_files_in_stage(tmpdir, conn_cnx, elem)


async def _put_list_rm_files_in_stage(tmpdir, conn_cnx, elem):
