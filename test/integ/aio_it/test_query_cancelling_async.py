...
@pytest.fixture()
async def conn_cnx_query_cancelling(request, conn_cnx):
    async with conn_cnx() as cnx:
        await cnx.cursor().execute("use role securityadmin")
    ...
    async with conn_cnx() as cnx:
        await cnx.cursor().execute("use role accountadmin")
        await cnx.cursor().execute("drop user magicuser1")
        await cnx.cursor().execute("drop user magicuser2")


async def _query_run(conn, shared, expectedCanceled=True):
    """Runs a query, and wait for possible cancellation."""
    async with conn(user="magicuser1", password="xxx") as cnx:
        await cnx.cursor().execute("use warehouse regress")

        # Collect the session_id
        async with cnx.cursor() as c:
            await c.execute("SELECT current_session()")
            async for rec in c:
                with shared.lock:
...
async def _query_cancel(conn, shared, user, password, expectedCanceled):
    """Tests cancelling the query running in another thread."""
    async with conn(user=user, password=password) as cnx:
        await cnx.cursor().execute("use warehouse regress")
        ...
        logger.info(
            "User %s's role is: %s",
            user,
            (await (await cnx.cursor().execute("select current_role()")).fetchone())[0],
        )
        ...
        try:
            ...
            await cnx.cursor().execute(query)
            assert (
                expectedCanceled
...
async def _test_helper(conn, expectedCanceled, cancelUser, cancelPass):
    ...
    queryRun = asyncio.create_task(_query_run(conn, shared, expectedCanceled))
    queryCancel = asyncio.create_task(
        _query_cancel(conn, shared, cancelUser, cancelPass, expectedCanceled)
    )
    await asyncio.gather(queryRun, queryCancel)


@pytest.mark.skipif(
    not CONNECTION_PARAMETERS_ADMIN, reason="Snowflake admin account is not accessible."
)
async def test_same_user_canceling(conn_cnx_query_cancelling):
    """Tests that the same user CAN cancel his own query."""
    await _test_helper(conn_cnx_query_cancelling, True, "magicuser1", "xxx")


@pytest.mark.skipif(
    not CONNECTION_PARAMETERS_ADMIN, reason="Snowflake admin account is not accessible."
)
async def test_other_user_canceling(conn_cnx_query_cancelling):
    """Tests that the other user CAN NOT cancel his own query."""
    await _test_helper(conn_cnx_query_cancelling, False, "magicuser2", "xxx")
