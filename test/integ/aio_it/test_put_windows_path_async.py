...
async def test_abc(conn_cnx, tmpdir, db_parameters):
    ...
    async with conn_cnx(
        ...
        rec = await (
            await con.cursor().execute(f"put file://{test_data} @~/{subdir}1/")
        ).fetchall()
        assert rec[0][6] == "UPLOADED"

        await con.cursor().execute(f"rm @~/{subdir}0")
        await con.cursor().execute(f"rm @~/{subdir}1")
