...
def _create_table(conn_cnx, columndefs, partial_name):
    table = '"check_data_integrity_{}"'.format(partial_name)
...
@pytest.mark.parametrize("columndefs, data_type, generator", [
    ...
    (('col1 DATE',), 'DATE', lambda i, j: DateFromTicks(time.time() + i * 86400 - j * 1313)),
    (('col2 STRING',), 'STRING', lambda i, j: random_string(1024, choices=string.ascii_letters + string.digits)),
    (('col2 TEXT',), 'TEXT', lambda i, j: ''.join([chr(i) for i in range(33, 127)] * 100)),
    (('col2 VARCHAR',), 'VARCHAR', lambda i, j: random_string(50, choices=string.ascii_letters + string.digits)),
    ...
    (('col1 TIMESTAMPTZ',), 'TIMESTAMPTZ', lambda i, j: _generate_time(i, j, "America/Vancouver")),
    (('col1 TIMESTAMP with time zone',), 'TIMESTAMPTZ_EXPLICIT',
     lambda i, j: _generate_time(i, j, "America/Vancouver")),
    (('col1 TIMESTAMPLTZ',), 'TIMESTAMPLTZ', lambda i, j: _generate_time(i, j, "America/New_York")),
    (('col1 TIMESTAMP_LTZ',), 'TIMESTAMP_fractional', lambda i, j: _generate_time(i, j, "Europe/Paris", True)),
    (('col1 TIME',), 'TIME', lambda i, j: TimeFromTicks(time.time() + i * 86400 - j * 1313))
])
