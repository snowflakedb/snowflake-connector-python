...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_okta_successful():
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_okta_with_wrong_okta_username():
    connection_parameters = AuthConnectionParameters().get_okta_connection_parameters()
    connection_parameters["user"] = "differentUsername"

    test_helper = AuthorizationTestHelper(connection_parameters)
    assert (
        not await test_helper.connect_and_execute_simple_query()
...
@pytest.mark.asyncio
async def test_okta_wrong_url_2():
    connection_parameters = AuthConnectionParameters().get_okta_connection_parameters()
