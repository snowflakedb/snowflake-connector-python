...
async def test_mfa_successful():
    ...
    connection_success = (
        await test_helper.connect_and_execute_simple_query_with_mfa_token(totp_codes)
    )
    ...
    cache_connection_success = (
        await cache_test_helper.connect_and_execute_simple_query()
    )
