import logging
from test.auth.aio.authorization_test_helper import (
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_snowflake_authorization_code_wildcards_successful():
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_snowflake_authorization_code_wildcards_mismatched_user():
    connection_parameters = (
        AuthConnectionParameters().get_snowflake_wildcard_external_authorization_code_connection_parameters()
    )
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_snowflake_authorization_code_wildcards_timeout():
    connection_parameters = (
        AuthConnectionParameters().get_snowflake_wildcard_external_authorization_code_connection_parameters()
    )
    test_helper = AuthorizationTestHelper(connection_parameters)
    ...
    assert (
        await test_helper.connect_and_execute_simple_query() is False
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_snowflake_authorization_code_wildcards_with_token_cache():
    connection_parameters = (
        AuthConnectionParameters().get_snowflake_wildcard_external_authorization_code_connection_parameters()
    )
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_snowflake_authorization_code_wildcards_without_token_cache():
    connection_parameters = (
        AuthConnectionParameters().get_snowflake_wildcard_external_authorization_code_connection_parameters()
    )
