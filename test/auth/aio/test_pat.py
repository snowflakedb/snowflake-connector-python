...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_authenticate_with_pat_successful() -> None:
    ...
    try:
        ...
        await test_helper.connect_and_execute_simple_query()
    finally:
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_authenticate_with_pat_mismatched_user() -> None:
    pat_command_variables = get_pat_setup_command_variables()
    connection_parameters = AuthConnectionParameters().get_pat_connection_parameters()
...
@pytest.mark.auth
@pytest.mark.asyncio
async def test_authenticate_with_pat_invalid_token() -> None:
    connection_parameters = AuthConnectionParameters().get_pat_connection_parameters()
...
async def get_pat_token(pat_command_variables) -> dict[str, Union[str, bool]]:
    ...
    pat_command_variables["token"] = (
        await test_helper.connect_using_okta_connection_and_execute_custom_command(
            command, True
        )
    )
...
async def remove_pat_token(pat_command_variables: dict[str, Union[str, bool]]) -> None:
    okta_connection_parameters = (
        AuthConnectionParameters().get_okta_connection_parameters()
    )
    ...
    test_helper = AuthorizationTestHelper(okta_connection_parameters)
    await test_helper.connect_using_okta_connection_and_execute_custom_command(command)
