...
class MockBlobServiceClient(BlobServiceClient):
    def get_blob_client(
        self,
        container,  # type: Union[ContainerProperties, str]
        blob,  # type: Union[BlobProperties, str]
        snapshot=None,  # type: Optional[Union[Dict[str, Any], str]]
    ):
        cln = BlobServiceClient.get_blob_client(self, container, blob, snapshot)
        return MockBlobClient(cln)
