...
def test_percentage(tmpdir):
    ...
    file_path = tmpdir / 'zero_file1'
    from pathlib import Path
    Path(file_path).touch()
    func_callback = SnowflakeS3ProgressPercentage(file_path.strpath, 0)
    func_callback(1)
    func_callback = SnowflakeAzureProgressPercentage(file_path.strpath, 0)
    func_callback(1)
