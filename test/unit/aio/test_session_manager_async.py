...
async def close_sessions(rest: SnowflakeRestful, num_session_pools: int) -> None:
    """Helper function to call SnowflakeRestful.close(). Asserts close was called on all SessionPools."""
    with mock.patch("snowflake.connector.aio._network.SessionPool.close") as close_mock:
        await rest.close()
...
async def create_session(
    ...
    if num_sessions == 0:
        return
    async with rest._use_requests_session(url):
        await create_session(rest, num_sessions - 1, url)


@mock.patch("snowflake.connector.aio._network.SnowflakeRestful.make_requests_session")
async def test_no_url_multiple_sessions(make_session_mock):
    rest = SnowflakeRestful(connection=mock_conn)

    await create_session(rest, 2)
    ...
    await close_sessions(rest, 1)


@mock.patch("snowflake.connector.aio._network.SnowflakeRestful.make_requests_session")
async def test_multiple_urls_multiple_sessions(make_session_mock):
    rest = SnowflakeRestful(connection=mock_conn)

    for url in [url_1, url_2, None]:
        await create_session(rest, num_sessions=2, url=url)
    ...
    await close_sessions(rest, 3)


@mock.patch("snowflake.connector.aio._network.SnowflakeRestful.make_requests_session")
async def test_multiple_urls_reuse_sessions(make_session_mock):
    rest = SnowflakeRestful(connection=mock_conn)
    for url in [url_1, url_2, url_3, None]:
        # create 10 sessions, one after another
        for _ in range(10):
            await create_session(rest, url=url)
