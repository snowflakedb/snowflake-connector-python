...
from snowflake.connector.aio.auth import Auth, AuthByKeyPair
from snowflake.connector.constants import OCSPMode
from snowflake.connector.description import CLIENT_NAME, CLIENT_VERSION


def _create_mock_auth_keypair_rest_response():
    async def _mock_auth_key_pair_rest_response(url, headers, body, **kwargs):
        return {
            "success": True,
            "data": {
                "token": "TOKEN",
...
async def test_auth_keypair_bad_type():
    """Simple Key Pair test using abstraction layer."""
    account = "testaccount"
    user = "testuser"
...
@patch("snowflake.connector.aio.auth.AuthByKeyPair.prepare")
async def test_renew_token(mockPrepare):
