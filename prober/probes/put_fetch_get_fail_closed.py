        ...
        with connect(connection_parameters) as conn:
            with conn.cursor() as cur:
                ...
                database_name = setup_database(
                    cur,
                    conn.database,
                    "cloudprober_driver_python_create_database_fail_closed",
                )
                logger.error("Database setup complete")

                logger.error("Setting up schema")
                schema_name = setup_schema(
                    cur,
                    conn.schema,
                    "cloudprober_driver_python_create_schema_fail_closed",
                )
                ...
                setup_warehouse(
                    cur,
                    conn.warehouse,
                    "cloudprober_driver_python_setup_warehouse_fail_closed",
                )
                ...
                count_data_from_table(
                    ...
                    num_records,
                    cur,
                    "cloudprober_driver_python_data_transferred_completely_fail_closed",
                )
                ...
                compare_fetched_data(
                    ...
                    file_name,
                    cur,
                    metric_name="cloudprober_driver_python_data_integrity_fail_closed",
                )
                ...
                execute_get_command(
                    stage_name,
                    conn,
                    "cloudprober_driver_python_perform_get_fail_closed",
                )
