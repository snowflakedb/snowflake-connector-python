...
- Upcoming Release
  ...
  - Fixed lazy-initialization races in the Arrow iterator C++ backend that could trigger on Python 3.13t/3.14t (`--disable-gil`) when multiple threads first exercised a converter concurrently. Specifically wrapped the one-shot Python module/object caches in `Logger::log` (`m_pyLogger`), `DateConverter::initPyDatetimeDate`, `TimeConverter::m_pyDatetimeTime`, and `DecimalBaseConverter::initPyDecimalConstructor` in `std::call_once`. `PyGILState_Ensure` does not provide mutual exclusion on free-threaded builds, so the previous check-then-set pattern could torn-write the cached pointer and leak the losing PyObject. Also drive-by-fixed two pre-existing reference leaks in `Logger::log` (the `Py_BuildValue("()")` positional-args tuple and the `PyObject_Call` return value were never decref'd).
  - Enabled `cibuildwheel`'s `cpython-freethreading` build set so `cp313t-*` / `cp314t-*` wheels are actually produced when the CI matrix targets them. Composes with #2850 (matrix entries) and the `freethreading_compatible` Cython directive above to deliver an end-to-end free-threaded wheel.
  - Added `test/unit/test_freethreading_import.py` to regress the GIL-state contract on Python 3.13+. Three subprocess-based tests assert: (1) no `RuntimeWarning` about the global interpreter lock is emitted at import time on any 3.13+ build, (2) on a free-threaded interpreter, `sys._is_gil_enabled()` stays `False` after import, and (3) on a GIL-enabled interpreter, it stays `True`. Together these give positive-and-negative acknowledgement that the Cython + C++ free-threading wiring works end-to-end and doesn't regress the GIL build.
  - Extended the conda recipe (`ci/anaconda/recipe/meta.yaml`) to produce a free-threaded `cp314t` `.conda` artifact when `FREETHREADING=1` is set. The new code path adds `track_features: py_freethreading` and pins host requirements to `python_abi *_cp314t` so the resulting package only resolves in a Python 3.14t environment. Existing GIL-build invocations (`conda build --python 3.14`) are unchanged.
