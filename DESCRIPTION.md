...
- v4.7.6 (Unreleased)
  ...
  - Fixed platform detection running again on every `connect()`. Its result was cached with the per-connection `SessionManager` clone as part of the key, so the cache never hit; it is now cached once per process (SNOW-3880829, #2983).
