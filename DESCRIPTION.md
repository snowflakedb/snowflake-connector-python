...
- Upcoming Release
  - Fixed `python-connector.log` not rotating on Windows, and every record being logged twice, when easy logging is enabled via `config.toml` (SNOW-3680325).
    - **Behavior change:** Easy logging no longer calls `logging.basicConfig()` and therefore no longer configures the root logger. `python-connector.log` now captures only the `snowflake.connector`, `botocore`, and `boto3` loggers. Logs from your own application or other libraries — which previously landed in the file by propagating to the root handler — are no longer written there, and the root logger level is no longer set. If you relied on this, attach your own handler to the root logger.
