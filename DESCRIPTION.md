...
# Release Notes
- Upcoming Release
  - Fixed `python-connector.log` not rotating on Windows, and every record being logged twice, when easy logging is enabled via `config.toml`. Easy logging now attaches a single shared rotating file handler to the `snowflake.connector`, `botocore`, and `boto3` loggers instead of also calling `logging.basicConfig()`. As a result it no longer configures the root logger: records from other libraries that previously reached `python-connector.log` by propagating to the root handler are no longer captured, and the root logger level is no longer set. Configure your own root logging if you relied on that (SNOW-3680325).
