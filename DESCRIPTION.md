...
- NEXT_RELEASE(TBD)
  ...
  - Stopped installing `boto3`/`botocore` by default. Install the optional extra with `pip install 'snowflake-connector-python[boto3]'` when AWS support is needed. The `SNOWFLAKE_NO_BOTO` install-time environment variable and the former `[boto]` extra were removed.
