...
- Upcoming Release
  ...
  - Fixed a bug where executing a fully-qualified DDL statement (e.g. `CREATE VIEW db.schema.obj`) on a session with no current schema would silently populate the connector's cached `_schema`/`_database`, causing `get_current_schema()` to diverge from the server's `CURRENT_SCHEMA()` and Snowpark temp-object qualification to target the wrong schema (SNOW-3665226).
